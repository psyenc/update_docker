...
class Config:
    def __init__(self):
        try:
            ...
            self.FFMPEG = config(
                "FFMPEG",
                default='ab-av1 auto-encode -i "{}" --enc map=-0:v:1? --enc map=-0:t? --acodec libopus --enc b:a=64k --scd true --svt scm=0:color-primaries=1:transfer-characteristics=1:matrix-coefficients=1:luminance-qp-bias=20 --keyint 193 --min-vmaf 87 --preset 5 --max-crf 70 --min-samples 3 --vfilter scale=-2:480:flags=bicubic:param0=0:param1=1/2 --downmix-to-stereo -o "{}"',
            )
            ...
            self.MUX_ARGS = config("MUX_ARGS", default='-i thumb2.jpg -map 0 -metadata title="" -metadata:s:v title="" -metadata:s:a title="" -metadata:s:s title="" -metadata:s:t mimetype=image/jpeg -attach thumb2.jpg')

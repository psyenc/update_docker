...
class Config:
    def __init__(self):
        try:
            ...
            self.FFMPEG = config(
                "FFMPEG",
                default='ab-av1 auto-encode -i "{}" --enc map=-0:v:1? --enc map=-0:t? --acodec libopus --enc b:a=64k --scd true --svt tile-columns=1:enable-dlf=2:enable-dg=0:mbr-overshoot-pct=0:scm=0:tune=0:color-primaries=1:transfer-characteristics=1:matrix-coefficients=1:qm-min=0:qm-max=15:chroma-qm-min=0:variance-boost-strength=1:variance-octile=4:tf-strength=1:luminance-qp-bias=25:sharpness=1:ac-bias=1.5:sharp-tx=1:hbd-mds=1:complex-hvs=1:noise-adaptive-filtering=0 --keyint 193 --min-vmaf 91 --preset 4 --max-crf 70 --min-samples 3 --vfilter scale=1280:-2:flags=bicubic:param0=0:param1=1/2 --downmix-to-stereo -o "{}"',
            )

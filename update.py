...
def update():
    ...
    UPSTREAM_REPO = config("UPSTREAM_REPO", default="https://github.com/Nubuki-all/Enc")
    ...
    cmd = f"git switch {UPSTREAM_BRANCH} -q \
        && git pull -q " "&& git reset --hard @{u} -q \
        && git clean -df -q"
    cmd2 = f"git init -q \
    ...
    if ALWAYS_DEPLOY_LATEST is True or update_check.is_file():
        ...
        if (
            os.path.exists(".git")
            and check_output(["git config --get remote.origin.url"], shell=True)
            .decode()
            .strip()
            == UPSTREAM_REPO
        ):
